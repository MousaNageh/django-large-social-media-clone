...
class LoginSerializerTestCase(TestCase):
    ...
    def test_valid_login_without_meta_data(self):
        data = {'username_or_email': self.user.email, 'password': self.user_data.get("password")}
        serializer = LoginSerializer(data=data, context={'request': self.factory.get('/')})
        serializer.is_valid()
        self.assertIn('not_allowed', serializer.errors)

    def test_non_existent_email(self):
        data = {'username_or_email': 'nonexistent@example.com', 'password': self.user_data.get("password")}
        serializer = LoginSerializer(data=data)
        with self.assertRaises(ValidationError):
            serializer.is_valid(raise_exception=True)

    def test_non_existent_username(self):
        data = {'username_or_email': 'nonexistusername', 'password': self.user_data.get("password")}
        serializer = LoginSerializer(data=data)
        with self.assertRaises(ValidationError):
            serializer.is_valid(raise_exception=True)

...
class ResendOTPSerializerTestCase(TestCase):
    ...
    def setUpTestData(cls):
        cls.active_user = get_user_model().objects.create_user(
            **get_user_object(is_active=True)
        )
        cls.inactive_user = get_user_model().objects.create_user(
            **get_user_object(is_active=False)
        )

    def test_valid_username_input(self):
        serializer = ResendOTPSerializer(
            data={"username_or_email": self.inactive_user.username}
        )
        self.assertTrue(serializer.is_valid(), serializer.errors)

    def test_valid_email_input(self):
        serializer = ResendOTPSerializer(
            data={"username_or_email": self.inactive_user.email}
        )
        self.assertTrue(serializer.is_valid(), serializer.errors)
    ...
    def test_nonexistent_email(self):
        serializer = ResendOTPSerializer(
            data={"username_or_email": "nonexistent@example.com"}
        )
        self.assertFalse(serializer.is_valid())
        self.assertIn("user_not_exists", serializer.errors)
    ...
    def test_user_already_activated_with_username(self):
        serializer = ResendOTPSerializer(
            data={"username_or_email": self.active_user.username}
        )
        self.assertFalse(serializer.is_valid())
    ...
    def test_user_already_activated_with_email(self):
        serializer = ResendOTPSerializer(
            data={"username_or_email": self.active_user.email}
        )
        self.assertFalse(serializer.is_valid())
        self.assertIn("user_already_activated", serializer.errors)
